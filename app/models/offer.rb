class Offer < ActiveRecord::Base
  ...
  attr_accessible :description,:season, :positions, :season_id, :position_ids,:min_league_id,
                  :max_age, :min_age, :min_league,:league_id, :team_type_id, :strong_foot, :valid_from, :valid_until
  ...
  scope :by_league_ranking, lambda { |ranking|
    joins(:league).where ("leagues.ranking <= #{ranking}") unless ranking.blank? }

  scope :by_team_type, lambda { |type| where ("team_type_id = #{type}") unless type.blank? }
